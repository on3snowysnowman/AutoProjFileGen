...
int main()
{
    ...
    // Name of the file the user wants to create.
    std::string new_file_name;

    // Use iostream to get the name of the new file.
    ...
    // If either header or cpp directory already exists.
    if(FileSystemHandler::does_directory_exist(include_directory) ||
        FileSystemHandler::does_directory_exist(cpp_directory))
    {
        std::cout << "[ERR] Header or Cpp directory already exists! Creating a new file would "
            "overwrite them.\n";
        exit(1);
    }
    ...
    std::string date = TimeObserver::get_date();
